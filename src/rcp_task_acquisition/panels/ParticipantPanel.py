...
class ParticipantPanel:
    ...
    def _setup_data(self):
        ...
        self.first_name_box = wx.TextCtrl(
            self.panel, size=wx.Size(150, -1), style=wx.TE_LEFT, value=""
        )
        last_name_text = wx.StaticText(self.panel, label="Last Name:")
        self.last_name_box = wx.TextCtrl(
            self.panel, size=wx.Size(150, -1), style=wx.TE_LEFT, value=""
        )
        ...
        self.diagnosis_box = wx.TextCtrl(
            self.panel, size=wx.Size(150, -1), style=wx.TE_LEFT, value=""
        )
        ...
        row_sizer.Add(
            first_name_text, pos=(1, 0), span=(1, 1), flag=wx.ALIGN_RIGHT | wx.ALL, border=5
        )
        row_sizer.Add(
            self.first_name_box, pos=(1, 1), span=(1, 2), flag=wx.ALIGN_LEFT | wx.ALL, border=5
        )
        row_sizer.Add(
            last_name_text, pos=(1, 3), span=(1, 1), flag=wx.ALIGN_RIGHT | wx.ALL, border=5
        )
        row_sizer.Add(
            self.last_name_box, pos=(1, 4), span=(1, 2), flag=wx.ALIGN_LEFT | wx.ALL, border=5
        )
        ...
        row_sizer.Add(
            diagnosis_text, pos=(2, 3), span=(1, 1), flag=wx.ALIGN_RIGHT | wx.ALL, border=5
        )
        row_sizer.Add(
            self.diagnosis_box, pos=(2, 4), span=(1, 2), flag=wx.ALIGN_LEFT | wx.ALL, border=5
        )
        return row_sizer
    ...
    def show_error(self):
        error = wx.MessageDialog(None, "Participant ID Required", "Error", wx.OK | wx.ICON_WARNING)
        error.ShowModal()
        error.Destroy()
    ...
    def add_event(self, event):
        ...
        add_success, participant_id = participantDB.add_participant(
            participant_id, first_name, last_name, age, diagnosis
        )

        if not add_success:
            logger.debug("Error adding participant")
            error = wx.MessageDialog(
                None, "Participant ID in use, please use different ID", "", wx.OK | wx.ICON_WARNING
            )
            error.ShowModal()
            error.Destroy()
