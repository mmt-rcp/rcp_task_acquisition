...
def graph_csv():
    with open(
        "D:/RawDataLocal/20260401/unitME/session011/20260401_unitME_session011_labjack.txt", "r"
    ) as csvfile:
        test_dict = {"PHOTODIODE": [], "CAMERA": []}
...
def run_graph():
    ...
    # print(digital)
    digital_reshape = np.unpackbits(digital.view(np.uint8)).reshape(8, len(y_values), order="F")[
        ::-1
    ]
